...
不同浏览器功能不同，比如我们写代码基本上都用 ES6 语法 import 和 export 导入导出

- 现代浏览器可以通过 `<script type="module"></script>`来支持 import export
- IE8-IE15 是不支持的 所以不能运行

怎么解决呢

1. 把代码直接全部放到`<script type="module"></script>` 缺点是 IE8-IE15 不支持 而且会导致文件请求过多，如果两个模块文件相互引用，当我打开 a 模块 会请求 b 模块，当我打开 b 模块（此时已经打开过 a 模块）由于代码引入了 a 模块 会再次请求 a 模块 这里的模块都是指文件
2. 把关键字转译成普通代码，把所有文件打包成一个文件 （这个就是 webpack 做的事情）
